...
(function () {
  ...
  async function sendToBackend(userText) {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Bad response from agent backend");
      ...
      if (data.lead) {
        state.lead.name = data.lead.name || state.lead.name;
        state.lead.email = data.lead.email || state.lead.email;
        state.lead.need = data.lead.need || state.lead.need;
        maybeShowBookingLink();
      }
    } catch (err) {
      removeTyping();
      addMessage(
